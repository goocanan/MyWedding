          ...
          >
            {items.map(({ value, label }) => (
              <motion.div key={label} className="w-32 md:w-40" variants={scaleIn}>
                <div className="gold-nameplate p-0 aspect-square flex flex-col items-center justify-center relative group overflow-hidden">
                   <div className="relative z-10 font-script text-5xl md:text-7xl text-gold-gradient drop-shadow-md">
                      {String(value).padStart(2, '0')}
                   </div>
                   <div className="relative z-10 font-serif-readable italic text-gold/60 uppercase tracking-widest text-xs mt-2">
                      {label}
                   </div>
                </div>
              </motion.div>
